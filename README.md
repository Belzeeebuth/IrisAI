...
[![Phase](https://img.shields.io/badge/phase-1%20%C2%B7%20prototype%20vocal-8b5cf6)](#roadmap)
[![Python](https://img.shields.io/badge/python-3.11%2B-3776ab)](pyproject.toml)
[![Licence](https://img.shields.io/badge/licence-MIT-green)](LICENSE)
[![Local](https://img.shields.io/badge/donn%C3%A9es-100%25%20locales-0ea5e9)](docs/PRIVACY.md)

</div>

Iris est un assistant vocal conçu pour [Omarchy](https://omarchy.org) (Arch Linux + Hyprland). Elle se lance avec ta session, écoute en arrière-plan le mot d'activation **« Hey Iris »**, comprend ce que tu dis en français ou en anglais, agit sur le système (applications, workspaces Hyprland, volume, luminosité, thèmes Omarchy, musique, verrouillage…) et te répond avec une voix naturelle. Tout tourne **en local** : ni la voix, ni le texte ne quittent la machine, sauf si tu l'autorises explicitement.

> **État du projet — Phase 1 (prototype vocal).** Le cœur (compréhension, actions, machine à états, journal, CLI, service systemd) est fonctionnel et couvert par 186 tests. La chaîne audio (micro → VAD → Whisper → Piper) est implémentée avec plusieurs backends de repli mais **n'a pas encore été validée sur une machine Omarchy physique** : les premiers retours de terrain serviront à ajuster les seuils (VAD, tolérance du mot d'activation). Voir [Limitations connues](#limitations-connues).
...
| **Contrôle** | « stop » · « mets-toi en pause pendant 10 minutes » · « reprends l'écoute » | |
...
```
   ...
   NLU  ── règles fr/en + commandes perso ──▶ Intent(volume_up)
     │
     ▼
   Routeur ── confirmation si critique ──▶ Action (wpctl / hyprctl / uwsm / omarchy-*)
     │                                          │
     ▼                                          ▼
   Journal SQLite (traçabilité)            Réponse ──▶ TTS Piper ──▶ pw-play
```

Machine à états : `IDLE` → (« Hey Iris ») → `ACTIVE` → (commande) → exécution → `ACTIVE` (fenêtre d'enchaînement) ou `IDLE`. Les intentions critiques passent par `CONFIRMING` (oui / non / délai de 12 s). « Mets-toi en pause » bascule en `PAUSED`. Pendant qu'Iris parle, le micro est ignoré pour qu'elle ne s'entende pas elle-même. Détails : [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
...
Le script : installe les paquets système manquants (`pacman`), crée un environnement Python isolé (`uv`, Python 3.12) dans `~/.local/share/iris/venv`, installe Iris avec `faster-whisper`, `piper-tts` et `sounddevice`, télécharge la voix française `fr_FR-siwis-medium` et le modèle Whisper `base`, crée `~/.config/iris/config.toml`, installe et démarre le service systemd, puis lance `iris doctor`.

Options : `--no-service`, `--no-voice`, `--no-pacman`, `--extras=stt,tts,audio,wakeword`. Variables : `IRIS_VOICE`, `IRIS_WHISPER_MODEL`, `IRIS_PYTHON`.
...
iris config init
iris voices download fr_FR-siwis-medium
iris models download base            # ou small (meilleur en français, ~2× plus lent)
iris service install
```

Extras disponibles : `stt` (faster-whisper), `tts` (piper-tts), `audio` (sounddevice + webrtcvad), `wakeword` (openWakeWord), `cloud` (requests, pour ElevenLabs / fallback Whisper API), `all`. Sans `audio`, Iris capture le micro avec `parec` (livré avec `libpulse`). Sans `tts`, elle se rabat sur `espeak-ng` puis sur l'affichage console.
...
piper_length_scale = 0.95   # < 1 : parle un peu plus vite
...
- Options cloud, toutes en opt-in explicite : ElevenLabs (voix), API Whisper d'OpenAI (repli STT). Le pont Claude Code (phase 3) envoie à Claude *uniquement* le texte de la demande « demande à Claude … », via ton abonnement.
...
├── audio/            capture, VAD, lecture, bips    ├── actions/
├── wakeword/         transcript (flou) / openWakeWord│   ├── system.py      run / launch (uwsm, systemd-run)
├── stt/              faster-whisper, fallback cloud  │   ├── hyprland.py    hyprctl
├── tts/              piper, elevenlabs, espeak, voix │   ├── apps.py        alias + .desktop, ouvrir/fermer
├── nlu/              normalisation, nombres, règles  │   ├── audio_ctl.py   wpctl / pactl
├── agents/           pont Claude Code (expérimental) │   ├── brightness.py, media.py, power.py, web.py
└── data/             config par défaut, alias apps   │   └── omarchy.py     thèmes, lock, screenshot, webapps
tests/                186 tests (NLU, wake, routeur, machine à états, VAD, config, journal, CLI)
scripts/install.sh    installation Omarchy            systemd/iris.service   hypr/iris-autostart.conf
```
...
- [x] **Phase 1 — Prototype vocal** *(cette version)* : activation « Hey Iris » (transcription floue, openWakeWord optionnel), transcription locale Whisper, VAD, commandes simples fr/en, voix Piper, confirmations, journal SQLite, mode texte, CLI, diagnostic, service systemd, script d'installation Omarchy, README.
...
- [ ] **Phase 2 — Intégration système** : dictée dans l'application active (`wtype`), widget d'état Waybar/Quickshell, notifications riches, gestion multi-écrans, contrôle des périphériques (Bluetooth, réseau), sessions de workspaces nommées.
- [ ] **Phase 3 — Agents IA + mémoire** : conversation libre via **Claude Code (abonnement claude.ai, pas d'API)**, agents en arrière-plan avec surveillance et notification à la fin (« surveille la compilation »), mémoire contextuelle (projets ouverts, tâches en cours), proposition de reprise (« reprendre ta session d'hier ? »).
...
- Le pont Claude Code est synchrone (Iris attend la réponse, jusqu'à `agents.claude_timeout_s`).
...
.venv/bin/pytest            # 186 tests, ~2 s, aucun matériel requis
