...
- **Un « hey » écorché n'est plus une commande** : Whisper transcrit souvent « Hey Iris » en « Heille Iris », « Heide Iris », « Hurt, Iris ». Le mot d'activation « iris » était alors reconnu en fin de phrase et le « hey » massacré passait pour un ordre — exécuté en silence, sans bip : Iris semblait sourde alors qu'elle avait entendu. Un mot unique placé **avant** le nom d'activation et qui ne correspond à aucune intention est désormais traité comme une activation simple : elle bipe et ouvre sa fenêtre d'écoute. Les vrais ordres d'un mot (« stop Iris », « pause Iris ») restent des ordres.
- **Le silence du LLM ne passe plus pour une panne de micro** : quand le modèle choisit de ne rien répondre à une phrase qui portait le mot d'activation, Iris accuse réception (bip) et garde la fenêtre ouverte, au lieu de retomber muette en veille. Du bruit capté pendant la fenêtre d'écoute, sans son nom, reste silencieux.
- **`iris restart`** : relance le service et attend qu'Iris écoute vraiment de nouveau — elle est prête quand elle écrit un état *postérieur* à la relance, le fichier d'état survivant à l'arrêt du processus. Code de retour 1 avec la commande de journal si le service retombe ou ne répond pas dans le délai. Aussi `iris service restart`, et un raccourci Hyprland proposé dans `contrib/hypr/iris.conf`.
...
- **Instances Prism Launcher** : « lance l'instance Astraworld », « ouvre la partie Oasis 2.0 », « launch the world … ». Les instances sont lues dans `~/.local/share/PrismLauncher/instances/` — nom affiché tiré de `instance.cfg` — et rapprochées avec le même barème que les applications, donc « astra world » retrouve « Astraworld ». Quand aucune ne convient, Iris énumère celles qu'elle connaît au lieu de chercher une application de ce nom.
- **Adverbes entre le verbe et le nom** : « ferme complètement Steam », « lance direct Firefox », « ouvre tout de suite … ». Ils cherchaient jusqu'ici une application nommée « complètement steam ».
- **Modèle injoignable** : Iris le dit (« Je n'arrive pas à joindre le modèle ») au lieu de répondre « je n'ai pas compris », ce qui accusait la phrase entendue plutôt que le réseau. Une coupure franche — DNS, connexion refusée, socket fermé — est retentée une fois ; un délai dépassé ne l'est pas, pour ne pas doubler le silence. `llm.timeout_s` passe de 30 s à 15 s : un assistant vocal ne peut pas laisser trente secondes sans réponse.
